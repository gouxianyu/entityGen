...
public class ServiceGenerator {
	...
	private String packageName=Config.packageOutPath+".service";//包名
	...
	public void generateMapper() {
		for (int i = 0; i < ag.tables.size(); i++) {
			sb=new StringBuffer();
			generateClass(i);
			OutputUtil.outputFile(packageName, sb+"", ag.classNames.get(i)+"Service", "java");
		}
	}
		...
		for (int i = 0; i < tables.length; i++) {
			for (int j = 0; j < ag.tables.size(); j++) {
				if(tables[i].equalsIgnoreCase(ag.tables.get(j))) {
					sb=new StringBuffer();
					generateClass(i);
					OutputUtil.outputFile(packageName, sb+"", ag.classNames.get(j)+"Service", "java");
				}
			}
		}
	}
	...
	public void generateClass(int no) {
		...
		sb.append("\r\npublic interface "+ag.classNames.get(no)+"Service{\r\n\r\n");
